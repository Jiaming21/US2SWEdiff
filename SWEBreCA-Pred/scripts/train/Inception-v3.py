...
def main():
    ...
    parser.add_argument(
        "--data_root",
        type=str,
        default="/n/holylfs05/LABS/zhuang_lab/Lab/Jiaming/SWEBreCA-Pred/data",
    )
    parser.add_argument(
        "--save_root",
        type=str,
    ...
    parser.add_argument(
        "--real_swe_dir",
        type=str,
        default="/n/holylfs05/LABS/zhuang_lab/Lab/Jiaming/SWEBreCA-Pred/real_swe_train",
    )
    ...
    if os.path.isdir(args.real_swe_dir):
        ...
        print(f"[SKIP] Missing real_swe_dir: {args.real_swe_dir}")

    for ds in datasets:
        for src in sources:
            data_dir = os.path.join(args.data_root, ds, "generated_images", src)
            if not os.path.isdir(data_dir):
                print(f"[SKIP] Missing folder: {data_dir}")
                continue
            save_path = os.path.join(args.save_root, ds, src, weight_filename)
            train_one([data_dir, args.real_swe_dir], save_path, args, device, tag=f"{ds}_{src}+real_swe")

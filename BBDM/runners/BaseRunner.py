...
class BaseRunner(ABC):
    ...
    def train(self):
        ...
        try:
            ...
            self.latest_average_loss = float('inf')
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for train_batch in pbar:
                    ...
                    save_step_interval = getattr(self.config.training, 'save_step_interval', None)
                    if (
                        self.is_main_process
                        and save_step_interval
                        and self.global_step > 0
                        and self.global_step % int(save_step_interval) == 0
                    ):
                        with torch.no_grad():
                            self.logger(f"saving step checkpoint at global_step {self.global_step}...")
                            model_states, optimizer_scheduler_states = self.get_checkpoint_states(stage='epoch_end')
                            ckpt_dir = self.config.result.ckpt_path
                            torch.save(
                                model_states, os.path.join(ckpt_dir, f'step_model_{self.global_step}.pth')
                            )
                            torch.save(
                                optimizer_scheduler_states,
                                os.path.join(ckpt_dir, f'step_optim_sche_{self.global_step}.pth'),
                            )
                            torch.save(model_states, os.path.join(ckpt_dir, 'last_model.pth'))
                            torch.save(
                                optimizer_scheduler_states, os.path.join(ckpt_dir, 'last_optim_sche.pth')
                            )
                ...
                # validation
                average_loss = self.latest_average_loss
                if (epoch + 1) % self.config.training.validation_interval == 0 or (
                    ...
                    with torch.no_grad():
                        ...
                        self.latest_average_loss = average_loss
                        torch.cuda.empty_cache()
                        ...
                        with torch.no_grad():
                            ...
                            if self.config.args.save_top:
                                ...
                                if top_key not in self.topk_checkpoints:
                                    ...
                                    self.topk_checkpoints[top_key] = {"loss": self.latest_average_loss,
                                                                      'model_ckpt_name': model_ckpt_name,
                                                                      'optim_sche_ckpt_name': optim_sche_ckpt_name}

                                    print(f"saving top checkpoint: average_loss={self.latest_average_loss} epoch={epoch + 1}")
                                    torch.save(model_states,
                                               os.path.join(self.config.result.ckpt_path, model_ckpt_name))
                                    torch.save(optimizer_scheduler_states,
                                               os.path.join(self.config.result.ckpt_path, optim_sche_ckpt_name))
                                else:
                                    if self.latest_average_loss < self.topk_checkpoints[top_key]["loss"]:
                                        ...
                                        print(
                                            f"saving top checkpoint: average_loss={self.latest_average_loss} epoch={epoch + 1}")

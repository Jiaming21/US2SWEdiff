...
class ManifoldHyperConnection(nn.Module):
    def __init__(self, dims, channels):
        ...
        self.gate = conv_nd(dims, channels, channels, kernel_size=1, stride=1, padding=0)

    def forward(self, x, residual):
        x_norm = self.norm(x)
        residual_norm = self.norm(residual)
        gate = torch.sigmoid(self.gate(x_norm))
        return x + gate * residual_norm


class MHCResidualBlock(nn.Module):
    def __init__(self, dims, channels):
        super(MHCResidualBlock, self).__init__()
        self.conv1 = conv_nd(dims, channels, channels, kernel_size=3, stride=1, padding=1)
        self.silu = nn.SiLU()
        self.conv2 = conv_nd(dims, channels, channels, kernel_size=3, stride=1, padding=1)
        self.mhc = ManifoldHyperConnection(dims, channels)

...
class FeatureExtractor(nn.Module):
    def __init__(self, hint_channels, dim=2):
        ...
        self.layer2 = ResidualBlock(dim, 32, 32) # ResidualBlock: size and channels stay unchanged
        ...
        self.layer5 = ResidualBlock(dim, 256, 256) # Smooth growth: 16 32 64 128 256

    def forward(self, x):
        x = self.initial_conv(x)  # (B, 16, H, W)
        x = self.layer1(x)  # (B, 16, H, W)
        x = self.conv_before_res2(x)  # (B, 32, H, W)
        x = self.layer2(x)  # (B, 32, H, W)
        x = x.permute(0, 2, 3, 1)  # Change to (B, H, W, 32) for PatchMerging
        x = self.patch_merge1(x)  # (B, H/2, W/2, 64)
        x = x.permute(0, 3, 1, 2)  # Change back to (B, 64, H/2, W/2) for ResidualBlock
        x = self.conv_before_res3(x)  # (B, 64, H/2, W/2)
        x = self.layer3(x)  # (B, 64, H/2, W/2)
        x = x.permute(0, 2, 3, 1)  # Change to (B, H/2, W/2, 64) for PatchMerging
        x = self.patch_merge2(x)  # (B, H/4, W/4, 128)
        x = x.permute(0, 3, 1, 2)  # Change back to (B, 128, H/4, W/4) for ResidualBlock
        x = self.conv_before_res4(x)  # (B, 128, H/4, W/4)
        x = self.layer4(x)  # (B, 128, H/4, W/4)
        x = x.permute(0, 2, 3, 1)  # Change to (B, H/4, W/4, 128) for PatchMerging
        x = self.patch_merge3(x)  # (B, H/8, W/8, 256)
        x = x.permute(0, 3, 1, 2)  # Change back to (B, 256, H/8, W/8) for ResidualBlock
        x = self.conv_before_res5(x)  # (B, 256, H/8, W/8)
        x = self.layer5(x)  # (B, 256, H/8, W/8)
        return x
...
class ControlNet(nn.Module):
    def __init__(
        ...
        # ) # command + ? # stride skip may lose information # conv_nd only wraps standard convolution 
        # 1) no strided skip 2) use residual block 3) stride=2/maxpooling -> swin-transformer patch merging modular stage

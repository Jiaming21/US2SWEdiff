#!/usr/bin/env python3
"""
Evaluate all metrics for Dispersive Loss Location experiments.
...
Generated:   MDHI_dispersive_upto_{k}_w{w}/images/test/images (b-000000_idx-0.png, ...).
Supports upto_0 .. upto_13 (and any other w, e.g. upto_1_w0.1).
...
def _dispersive_sort_key(name: str):
    """Sort by (upto_k, w): upto_0_w0.05, upto_1_w0.05, ..., upto_13_w0.05, upto_1_w0.1, ..."""
...
def discover_dispersive_dirs():
    """Find all MDHI_dispersive_upto_*_w* with images/test/images (upto_0..13 and any w)."""
    dirs = []
    for d in DISPERSIVE_ROOT.iterdir():
        if not d.is_dir():
            continue
        if not d.name.startswith("MDHI_dispersive_upto_"):
            continue
        img_dir = d / "images" / "test" / "images"
        if img_dir.exists():
            dirs.append((d.name, img_dir))

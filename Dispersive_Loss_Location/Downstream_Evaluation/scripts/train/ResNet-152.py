#!/usr/bin/env python3
"""
Self-contained ResNet-152 downstream training (same data paths as VGG-16.py), without relying on SWEBreCA-Pred/ResNet-50.py.
"""
import argparse
import os
import random
...
def infer_condition_for_source(src: str) -> str:
    if src.startswith("canny_"):
        return "canny"
    if src.startswith("laplacian_"):
        return "laplacian"
    if src.startswith("us_"):
        return "us"
    # Backward compatibility for old dispersive runs.
    return "laplacian"
...
def _build_resnet152(num_classes: int = 2) -> nn.Module:
    try:
        w = models.ResNet152_Weights.DEFAULT
...
def main():
    parser = argparse.ArgumentParser(description="Train ResNet-152 (self-contained, same data layout as VGG-16).")
    parser.add_argument(
        "--project_root",
        type=str,
        default=BASE,

...
DEFAULT_SOURCES = [
    ...
    "canny_mdhi_no_disp",
    "canny_original_disp_upto_8",
    "canny_original_no_disp",
    "laplacian_mdhi_disp_upto_8",
    "laplacian_mdhi_no_disp",
    "laplacian_original_disp_upto_8",
    "laplacian_original_no_disp",
    "us_mdhi_disp_upto_8",
    "us_mdhi_no_disp",
    "us_original_disp_upto_8",
    "us_original_no_disp",
]
...
def plot_single_roc(y_true: List[int], y_prob: List[float], out_path: str, title: str):
    if len(set(y_true)) < 2:
        print(f"[SKIP] ROC needs both classes: {out_path}")
        return
    fpr, tpr, _ = roc_curve(y_true, y_prob)
    auc_val = roc_auc_score(y_true, y_prob)
    plt.figure(figsize=(6, 5), dpi=150)
    plt.plot(fpr, tpr, label=f"AUROC = {auc_val:.4f}", linewidth=2)
...
def plot_combined_roc(curves: List[dict], out_path: str, title: str):
    valid = [c for c in curves if len(set(c["y_true"])) >= 2]
    if not valid:
        print(f"[SKIP] No valid curves for: {out_path}")
        return
    plt.figure(figsize=(7.5, 6), dpi=150)
    for c in valid:
        fpr, tpr, _ = roc_curve(c["y_true"], c["y_prob"])
        auc_val = roc_auc_score(c["y_true"], c["y_prob"])
        plt.plot(fpr, tpr, linewidth=1.8, label=f"{c['label']} (AUC={auc_val:.4f})")
    plt.plot([0, 1], [0, 1], linestyle="--", linewidth=1)
    plt.xlim(0.0, 1.0)
    plt.ylim(0.0, 1.0)
    plt.xlabel("False Positive Rate")
...
def plot_public_3x3_roc(rocs_root: str, epochs: int, sources: List[str]):
    ...
    for r, model_name in enumerate(models_list):
        for c, ds_name in enumerate(datasets):
            ...
            special_laplacian = "Real SWE + laplacian_mdhi_disp_upto_8"
            other_idx = 0
...
def main():
    parser = argparse.ArgumentParser(description="Infer Inception-v3 weights on real_swe_test and update summary.")
    parser.add_argument(
        "--save_root",
        type=str,
    ...
    for wp in weight_paths:
        ...
        if row:
            rows.append(row)
            probs_csv_path = row["Probs CSV Path"]
            y_true = []
            y_prob = []
            with open(probs_csv_path, "r", newline="") as f:
                for r in csv.DictReader(f):
                    y_true.append(int(r["y_true"]))
                    y_prob.append(float(r["prob_malignant"]))
            curves.append(
                {
                    "label": f"{row['US Dataset']} | {row['Training Dataset']}",
                    "y_true": y_true,
                    "y_prob": y_prob,
                }
            )

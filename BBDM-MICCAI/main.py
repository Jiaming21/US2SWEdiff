...
def parse_args_and_config():
    ...
    parser.add_argument('--sample_output_dir', type=str, default=None, help='direct output directory for sample_to_eval images')
    ...
    parser.add_argument('--save_step_interval', type=int, default=None, help='checkpoint save interval by step')
    # data
    parser.add_argument('--dataset_type', type=str, default='' ,help='dataset type (search register)')
    parser.add_argument('--dataset_path', type=str, default=None, help='dataset root path override')
    parser.add_argument('--metadata_name', type=str, default=None, help='metadata json/jsonl file name, e.g. metadata_us.json')
    parser.add_argument('--source_modality', type=str, default=None, help='source modality tag, e.g. us/canny/laplacian')
    parser.add_argument('--target_modality', type=str, default=None, help='target modality tag, default swe')
    parser.add_argument('--valid_split', type=str, default=None, help='validation split folder name')
    parser.add_argument('--test_split', type=str, default=None, help='test split folder name')
    ...
    if args.save_step_interval is not None:
        namespace_config.training.save_step_interval = args.save_step_interval
    # model
    ...
    if args.metadata_name is not None:
        namespace_config.data.dataset_config.metadata_name = args.metadata_name
    if args.source_modality is not None:
        namespace_config.data.dataset_config.source_modality = args.source_modality
    if args.target_modality is not None:
        namespace_config.data.dataset_config.target_modality = args.target_modality
    if args.valid_split is not None:
        namespace_config.data.dataset_config.valid_split = args.valid_split
    if args.test_split is not None:
        namespace_config.data.dataset_config.test_split = args.test_split

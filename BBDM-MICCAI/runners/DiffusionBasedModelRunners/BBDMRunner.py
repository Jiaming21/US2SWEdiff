...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        # Keep RGB visualization for PNG dataset; keep legacy middle-slice grayscale for volumetric datasets.
        use_rgb_visualization = self.config.data.dataset_type == 'breastca_png_aligned'
    ...
    def sample_to_eval(self, net, test_dataset, sample_path):
        ...
        # When sample_output_dir is set, write images directly there.
        direct_output = getattr(self.config.args, "sample_output_dir", None)
        ...
        # PNG paired dataset: save synthesized images directly as PNG and skip NIfTI-based metrics.
        if dataset_type == 'breastca_png_aligned':
            for idx in tqdm(range(len(test_dataset))):
                ...
                # Keep name in list form for compatibility with model.sample.
                if isinstance(x_cond_name, (bytes, str)):
                    x_cond_name_in = [x_cond_name]
                else:
                    x_cond_name_in = x_cond_name

                sample = net.sample(
                    x, x_cond, x_cond_name_in,
                    context=context,
                    clip_denoised=False,
                    path=sample_path,
                    save=False,
                ...
                # Decode filename and save one synthesized PNG.
                out_name = x_name
                if isinstance(out_name, (list, tuple)):
                    out_name = out_name[0]
                if isinstance(out_name, bytes):
                    out_name = out_name.decode('utf-8')
                out_name = os.path.basename(str(out_name))
                if not out_name.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.webp', '.tif', '.tiff')):
                    out_name = f"{out_name}.png"
                save_single_image(sample[0], sample_path, out_name, to_normal=self.config.data.dataset_config.to_normal)

            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for train_batch in pbar:
                    ...
                    with torch.no_grad():
                        ...
                        # Optional step-based checkpointing: save every N optimization steps.
                        save_step_interval = getattr(self.config.training, 'save_step_interval', None)
                        if save_step_interval is not None and save_step_interval > 0 and \
                                self.global_step % save_step_interval == 0:
                            if not self.config.training.use_DDP or \
                                ...
                                torch.save(model_states,
                                           os.path.join(self.config.result.ckpt_path,
                                                        f'step_model_{self.global_step}.pth'))
                                torch.save(optimizer_scheduler_states,
                                           os.path.join(self.config.result.ckpt_path,
                                                        f'step_optim_sche_{self.global_step}.pth'))

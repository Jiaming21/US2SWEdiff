#!/usr/bin/env python3
"""
Evaluate all metrics on 4000-step generated results (all conditions/variants), without benign/malignant split.
...
def _build_controlnet_pairs(condition: str, variant: str):
    ...
    return gen_paths, real_paths, prompts
...
def main():
    ...
    rows = []
    for condition in CONDITIONS:
        stats_name = f"generated_eval_4000_{condition}_full"
        # use the first available family as reference for real set stats
        _, real_paths_for_stats, _ = build_pairs("controlnet", condition, CONTROLNET_VARIANTS[0])
        if not real_paths_for_stats:
            _, real_paths_for_stats, _ = build_pairs("ct2mri", condition, "")
        if not real_paths_for_stats:
            _, real_paths_for_stats, _ = build_pairs("pix2pixhd", condition, "")
        if not real_paths_for_stats:
            log(f"Skip condition {condition}: no real/reference pairs found.")
            continue
        ensure_custom_stats(stats_name, real_paths_for_stats, num_workers=args.num_workers)

        for family, cond, variant in model_specs:
            if cond != condition:
                continue
            model_id = f"{family}_{condition}_{variant}" if variant else f"{family}_{condition}"
            gen_paths, real_paths, prompts = build_pairs(family, condition, variant)
            if not gen_paths:
                log(f"Skip {model_id}: no valid pairs.")
                continue
            n = len(gen_paths)
            row = {"family": family, "condition": condition, "variant": variant or "-", "model_id": model_id, "n_pairs": n}
            for k in ALL_METRIC_NAMES:
                row[k] = np.nan

            log(f"Model {model_id}: FID/KID...")
            ...
            psnr_val, ssim_val = compute_psnr_ssim(gen_paths, real_paths)
            row["PSNR"], row["SSIM"] = psnr_val, ssim_val
            row["LPIPS"] = compute_lpips(gen_paths, real_paths)

            log(f"  CLIP/CMMD...")
            clip_t, clip_i, cmmd = compute_clip_cmmd(gen_paths, real_paths, prompts, clip_model, clip_pre, clip_tok)
            row["CLIP-T"], row["CLIP-I"], row["CMMD"] = clip_t, clip_i, cmmd
            rows.append(row)

    cols = ["family", "condition", "variant", "model_id", "n_pairs"] + ALL_METRIC_NAMES
    df = pd.DataFrame(rows)
    for c in cols:
        if c not in df.columns:
            df[c] = np.nan
    df = df[cols]
    out_csv = RESULTS_DIR / "all_metrics_both.csv"

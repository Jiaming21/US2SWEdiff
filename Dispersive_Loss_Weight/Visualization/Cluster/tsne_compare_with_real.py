#!/usr/bin/env python3
"""
Plot 5 t-SNE figures: each weight setting vs Real SWE.
...
DISPERSIVE_ROOT = Path("/n/holylfs05/LABS/zhuang_lab/Lab/Jiaming/Dispersive_Loss_Weight")
WEIGHTS = ["0.01", "0.03", "0.05", "0.07", "0.09"]
METHOD_SPECS = {
    f"w{w}": {
        "root": DISPERSIVE_ROOT / f"MDHI_dispersive_upto_8_w{w}" / "images" / "test" / "images",
        "suffix": None,
    }
    for w in WEIGHTS
}
...
def main() -> None:
    ...
    for i, (method_name, spec) in enumerate(METHOD_SPECS.items()):
        ...
        out_path = out_dir / f"tsne_{method_name}.png"
    ...
    fig.suptitle("t-SNE: Dispersive Weights vs Real SWE (Unified Axis Scale)", fontsize=14)
